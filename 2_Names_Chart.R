...
best_kerasaurs <- c("Polycocalus", "Odoceratops", "Denversaura", "Phatyceratops",
                    "Eohadrosaurus", "Macroplatasaurus", "Morontosaurus", "Iguannasaurus",
                    "Gilakosaurus", "Trindosaurus", "Xiannansaurus", "Brachycoelus",
                    "Mimanosaurus", "Huaxialodon", "Xuanhualong")
...
dat_dist <- as.data.frame(adist(kerasaurs, phylo)) %>% 
  ...
  filter(kerasaur %in% best_kerasaurs) %>% 
...
dat_chart <- dat_dist %>% 
  ...
  mutate(tree_x = sample(c(1, rep(2, 3), rep(3, 5), rep(4, 9)), n_kerasaurs, replace = FALSE)) %>% 
...
# dat_phylo_raster %>% 
#   filter(kerasaur == "Angalcaraus") %>% 
#   ggplot(aes(x=x, y=y), fill = "#00436b", alpha = 0.7)+
#   geom_raster()+
#   coord_fixed()
...
dat_phylo %>% 
  ...
  geom_label(aes(label = kerasaur), 
             ...
             fontface = "bold") +
  scale_x_continuous(breaks = x_time, labels = x_axis, name = "Million Years") + 
  coord_fixed(xlim = c(y_set*2, y_set*20)) +
  labs(title = "Kerasauria phylogenetic tree",
