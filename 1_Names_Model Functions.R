...
iterate_model <- function(model, characters, max_length, diversity, vectors, iterations){
  for(iteration in 1:iterations){
    ...
    for(diversity in c(0.2, 0.5, 1, 1.2)){
      ...
      current_saur <- 1:10 %>% map_chr(function(x) generate_saur(model, characters,max_length, diversity))
      
      cat(current_saur, sep="\n")
      cat("\n\n")
...
# this function takes a model, the characters, and a set diversity and generates a _saur
# from it. We have to use special logic to stop when we see the stop character
generate_saur <- function(model, characters, max_length, diversity){
